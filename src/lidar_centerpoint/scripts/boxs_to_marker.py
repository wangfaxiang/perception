...
class BoxsToMarker(Node):
    def __init__(self):
        ...
        # 类别颜色映射（按 ID，与 lidar_centerpoint 5 类模型一致）
        self.colors = {
            0: (0.0, 1.0, 0.0),       # CAR 绿
            1: (0.0, 0.5, 1.0),       # TRUCK 蓝
            2: (1.0, 1.0, 0.0),       # BUS 黄
            3: (0.5, 1.0, 0.0),       # BICYCLE 黄绿
            4: (1.0, 0.0, 0.0),       # PEDESTRIAN 红
        }
        # 类别名称映射
        self.class_names = {
            0: 'CAR', 1: 'TRUCK', 2: 'BUS', 3: 'BICYCLE', 4: 'PEDESTRIAN'
        }
    ...
    def callback(self, msg: Boxs):
        ...
        for i, box in enumerate(msg.box):
            ...
            name = box.label if box.label else self.class_names.get(box.id, '?')
            label_text = f"{name} {box.score:.2f}"
