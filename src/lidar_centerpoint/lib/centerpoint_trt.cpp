...
: config_(config)
{
  ...
  if (!encoder_trt_ptr_->init(
        encoder_param.onnx_path(), encoder_param.engine_path(), encoder_param.trt_precision())) {
    throw std::runtime_error("Failed to initialize the encoder TensorRT engine.");
  }
  ...
  if (!head_trt_ptr_->init(
        head_param.onnx_path(), head_param.engine_path(), head_param.trt_precision())) {
    throw std::runtime_error("Failed to initialize the head TensorRT engine.");
  }
