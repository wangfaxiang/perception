  ...
  const size_t workspace_size)
{
  ...
  auto parser =
    TrtUniquePtr<nvonnxparser::IParser>(nvonnxparser::createParser(*network, logger_));
  if (!parser->parseFromFile(
        onnx_path.c_str(), static_cast<int>(nvinfer1::ILogger::Severity::kERROR))) {
    RCLCPP_ERROR(
      rclcpp::get_logger("lidar_centerpoint"), "Failed to parse ONNX: %s", onnx_path.c_str());
    return false;
  }
  ...
  plan_ = TrtUniquePtr<nvinfer1::IHostMemory>(builder->buildSerializedNetwork(*network, *config));
  if (!plan_) {
