...
inline double normalizeRadian(double rad, double min_rad = -M_PI)
{
  const double max_rad = min_rad + 2.0 * M_PI;
  const double value = std::fmod(rad, 2.0 * M_PI);
  if (min_rad <= value && value < max_rad) return value;
  return value - std::copysign(2.0 * M_PI, value - max_rad + 1e-9);
}

inline double normalizeDegree(double deg, double min_deg = 0.0)
{
  const double max_deg = min_deg + 360.0;
  const double value = std::fmod(deg, 360.0);
  if (min_deg <= value && value < max_deg) return value;
  return value - std::copysign(360.0, value - max_deg + 1e-9);
}

template <typename T>
inline float calcDistance3d(const T & p1, const T & p2)
{
  float dx = p1.x - p2.x;
  float dy = p1.y - p2.y;
  float dz = p1.z - p2.z;
...
/// Simplified get_param replacement (from pointcloud_preprocessor)
template <typename T>
bool get_param(const std::vector<rclcpp::Parameter> & p, const std::string & name, T & value)
{
  auto it = std::find_if(p.begin(), p.end(), [&name](const rclcpp::Parameter & param) {
...
: Node("scan_ground_filter", options)
{
  // set initial parameters
  {
    ...
    // Simple vehicle info from parameters
    vehicle_info_.wheel_base_m =
      static_cast<float>(declare_parameter("wheel_base", 2.7));
    vehicle_info_.vehicle_height_m =
      static_cast<float>(declare_parameter("vehicle_height", 1.5));
